...
def index():
    ...
    responce_data = Asset.query.with_entities(Asset.ticker, Asset.name).order_by(Asset.name).all()
...
def asset_selection():
    ...
    if form.validate_on_submit():
        ...
        # flash(f'{form.asset.data} : {form.number.data } : {current_user.id} : {assetid}')
        ...
        db.session.commit()
        flash(f'Success data insertion in database for {ticker}')
